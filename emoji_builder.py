...
def get_glyph_name_from_gsub (codes, font, cmap_dict):
	ligatures = font['GSUB'].table.LookupList.Lookup[0].SubTable[0].ligatures
	first_glyph = cmap_dict[int(codes[0],16)]
	rest_of_glyphs = [cmap_dict[int(code,16)] for code in codes[1:]]
...
"""
	...
	for img_prefix in img_prefixes:
		...
		for u, img_file in img_files.items ():
			uchars = u.split("-")
			if len (uchars) == 1:
				glyph_name = unicode_cmap.cmap[int(uchars[0], 16)]
			else:
